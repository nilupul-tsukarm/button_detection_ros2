...
class ButtonDetector:
  ...
  def init_detector(self):
    ...
    with detection_graph.as_default():
      od_graph_def = tf.compat.v1.GraphDef()  # Use tf.compat.v1.GraphDef
      with tf.io.gfile.GFile(self.graph_path, 'rb') as fid:  # Use tf.io.gfile.GFile instead of tf.gfile.GFile
          serialized_graph = fid.read()
          od_graph_def.ParseFromString(serialized_graph)
          tf.import_graph_def(od_graph_def, name='')

    # Create a session with tf.compat.v1.Session
